...
/**
 * @file knoQSortPointer.c
 * @brief Kernel Layer Quick Sort implementation routines
 */
...
static void knoQSort2Way( void            ** aList,
                          stlInt32           aElemCount,
                          knlQSortContext  * aContext )
{
    void        * sStackList[KNO_QSORT_STACK_SIZE];
    ...
    while( STL_TRUE )
    {
        if( sElemCount < KNO_QSORT_CUTOFF )
        {
            ...
            {
                if( aContext->mCompare(aContext->mCmpInfo, &sList[i - 1], &sList[i]) <= 0 )
                {
                    continue;
                }
                ...
                for( j = i - 1; j > 0; j-- )
                {
                    if( aContext->mCompare(aContext->mCmpInfo, &sList[j - 1], &sSave) <= 0 )
                    {
                        break;
                    }
                    sList[j] = sList[j - 1];
        ...
        else
        {
            ...
            if( aContext->mCompare(aContext->mCmpInfo, &sList[0], &sList[sMid]) > 0 )
            {
                KNO_QSORT_SWAP_POINTER( sList[0], sList[sMid], sSave );
            }

            if( aContext->mCompare(aContext->mCmpInfo, &sList[0], &sList[sElemCount -1]) > 0 )
            {
                KNO_QSORT_SWAP_POINTER( sList[0], sList[sElemCount -1], sSave );
            }

            if( aContext->mCompare(aContext->mCmpInfo, &sList[sMid], &sList[sElemCount -1]) > 0 )
            {
                KNO_QSORT_SWAP_POINTER( sList[sMid], sList[sElemCount -1], sSave );
            }
            ...
            {
                while( aContext->mCompare(aContext->mCmpInfo, &sList[sCurLeft], &sPivotVal) < 0 )
                {
                    sCurLeft++;
                }
                while( aContext->mCompare(aContext->mCmpInfo, &sList[sCurRight], &sPivotVal) > 0 )
                {
                    sCurRight--;
                }
                ...
                {
                    KNO_QSORT_SWAP_POINTER( sList[sCurLeft], sList[sCurRight], sSave );
                    sCurLeft++;
                    sCurRight--;
                }
            }
            ...
            if( (sCurRight + 1) < (sElemCount - sCurLeft) )
            {
                ...
                sStackPos++;
                sStackList[sStackPos]       = (void*)(&sList[sCurLeft]);
            ...
            else
            {
                ...
                sStackPos++;
                sStackList[sStackPos]       = (void*)sList;
        ...
        {
            sList       = (void**)sStackList[sStackPos];
...
static void knoQSort3Way( void            ** aList,
                          stlInt32           aElemCount,
                          knlQSortContext  * aContext )
{
    void        * sStackList[KNO_QSORT_STACK_SIZE];
    ...
    while( STL_TRUE )
    {
        ...
        else
        {
            ...
            if( aContext->mCompare(aContext->mCmpInfo, &sList[sElemCount -1], &sList[sMid]) > 0 )
            {
                KNO_QSORT_SWAP_POINTER( sList[sMid], sList[sElemCount -1], sSave );
            }
            ...
            while( (sCurLeft < sElemCount - 1) && (sCurLeft < sRightEqual) )
            {
                ...
                if( sRet < 0 )
                {
                    ...
                    {
                        KNO_QSORT_SWAP_POINTER( sList[sCurLeft], sList[sLeftSmall], sSave );
                    }
                    sCurLeft++;
                    sLeftSmall++;
                }
                ...
                {
                    sRightEqual--;
                    KNO_QSORT_SWAP_POINTER( sList[sCurLeft], sList[sRightEqual], sSave );
                }
                else
                {
                    sCurLeft++;
            ...
            else
            {
                ...
                for( i = 0; i < sMinLen; i++ )
                {
                    KNO_QSORT_SWAP_POINTER( sList[sLeftSmall + i], sList[sElemCount - 1 - i], sSave );
                }

                sCurLeft = sLeftSmall + sMinLen;
