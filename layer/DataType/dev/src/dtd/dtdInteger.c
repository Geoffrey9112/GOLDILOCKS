...
/**
 * @file dtdInteger.c
 * @brief DataType Layer Integer type 정의
 */
...
/**
 * @addtogroup dtlInteger
 * @{
 */

/**
 * @brief Integer 타입의 length값을 얻음
...
{
    *aLength = DTL_NATIVE_INTEGER_SIZE;
...
/**
 * @brief Integer 타입의 length값을 얻음 (입력 문자열에 영향을 받지 않음)
 * @param[in]  aPrecision    사용하지 않음
                                         ...
                                         stlErrorStack       * aErrorStack )
{
    ...
    /* string value로부터 Integer 타입의 value 구성 */
    ...
    STL_TRY_THROW( (sConvertedValue >= DTL_NATIVE_INTEGER_MIN) &&
                   (sConvertedValue <= DTL_NATIVE_INTEGER_MAX),
                   ERROR_OUT_OF_RANGE );
...
/**
 * @brief Integer 타입의 length값을 얻음 (입력 숫자에 영향을 받지 않음)
 * @param[in]  aPrecision  사용하지 않음
 * @param[in]  aScale      사용하지 않음
...
stlStatus dtdIntegerGetLengthFromReal( stlInt64          aPrecision,
                                       ...
                                       stlErrorStack   * aErrorStack )
{
    /* real value로부터 Integer 타입의 value 구성 */
    STL_TRY_THROW( (aReal >= DTL_NATIVE_INTEGER_MIN) &&
                   (aReal <= DTL_NATIVE_INTEGER_MAX),
                   ERROR_OUT_OF_RANGE );
...
/**
 * @brief string value로부터 Integer 타입의 value 구성
                                        ...
                                        stlErrorStack        * aErrorStack )
{
    ...
    *sDstValue = (dtlIntegerType) sConvertedValue;
...
/**
 * @brief integer value로부터 Integer 타입의 value 구성
...
/**
 * @brief real value로부터 Integer 타입의 value 구성
                                      ...
                                      stlErrorStack        * aErrorStack )
{
    ...
    *aSuccessWithInfo = STL_FALSE;
    ...
    if( aReal >= 0 )
    {
        *sDstValue = (dtlIntegerType) ( aReal + 0.5 );
    }
    else
    {
        *sDstValue = (dtlIntegerType) ( aReal - 0.5 );
                              ...
                              stlErrorStack   * aErrorStack )
{
    ...
    sValue = *((dtlIntegerType*) aValue->mValue);
...
/**
 * @brief Integer value를 사용가능한 버퍼사이즈에 맞게 잘라서 문자열로 변환한다.
 * @param[in]  aValue          dtlDataValue
 * @param[in]  aPrecision      precision
 * @param[in]  aScale          scale
 * @param[in]  aAvailableSize  aBuffer에 사용할 수 있는 메모리 공간의 크기
