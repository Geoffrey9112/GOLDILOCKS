...
/**
 * @file dtdDate.c
 * @brief DataType Layer Date type 정의
 */
...
/**
 * @brief Date 타입의 length값을 얻음 (입력 문자열에 영향을 받지 않음)
 * @param[in]  aPrecision  
 * @param[in]  aScale
 * @param[in]  aStringUnit   string length unit (dtlStringLengthUnit 참조) 
...
/* /\** */
/*  * @brief Date 타입의 length값을 얻음 (입력 숫자에 영향을 받지 않음) */
/*  * @param[in]  aPrecision   */
/*  * @param[in]  aScale       */
...
/* /\** */
/*  * @brief integer value로부터 Date 타입의 value 구성 */
                           ...
                           stlErrorStack   * aErrorStack )
{
    ...
    sToCharFormatInfo = aVectorFunc->mGetDateFormatInfoFunc( aVectorArg );
...
/**
 * @brief Date value를 사용가능한 버퍼사이즈에 맞게 잘라서 문자열로 변환한다.
 * @param[in]  aValue          dtlDataValue
 * @param[in]  aPrecision      precision
 * @param[in]  aScale          scale
 * @param[in]  aAvailableSize  aBuffer에 사용할 수 있는 메모리 공간의 크기
...
/**
 * @brief session format 정보를 이용하여 String으로부터 Date 타입의 value 구성
 ...
 * 
 * @remark
 * DATE type : 최대 년월일 시분초 까지 저장할 수 있다.
 * DATE type에 string으로 입력 가능한 포멧은 SESSION FORMAT 정보이다.
                                     ...
                                     stlErrorStack        * aErrorStack )
{
    ...
    DTL_PARAM_VALIDATE( aAvailableSize >= DTL_DATE_SIZE, aErrorStack );
    DTL_CHECK_TYPE( DTL_TYPE_DATE, aValue, aErrorStack );
    ...
    *aSuccessWithInfo = STL_FALSE;
    ...
    sToDateTimeFormatInfo =
        aSourceVectorFunc->mGetDateFormatInfoFunc( aSourceVectorArg );    
...
/**
 * @brief date의 typed literal 입력 문자열을 통해 mValue를 구성
 *    <BR> DATE'2014-10-27'
 *    <BR> format 형식은 SYYYY-MM-DD 가 적용된다. 
 * @param[in]  aString            value에 저장될 입력 문자열
 * @param[in]  aStringLength      aString의 길이   
 * @param[in]  aAvailableSize     mValue에 사용할 수 있는 메모리 공간의 크기  
 * @param[out] aValue             dtlDataValue의 주소
                                           ...
                                           stlErrorStack       * aErrorStack )
{
    ...
    /*
     * date typed literal 처리
     * ex) DATE'2014-10-27'
     *     format 형식은 SYYYY-MM-DD가 적용된다.
     */ 

    /* SYYYY-MM-DD 정보를 얻는다. */
    STL_TRY( dtfToDateInfoFromTypedLiteral( DTL_TYPE_DATE,
                                            sDateTimeStr,
                                            sDateTimeStrLen,
                                            & sDtlExpTime,
                                            & sDateTimeStr,
...
/**
 * @brief session format 정보를 이용하여 Utf16 String으로부터 Date 타입의 value 구성
 * @param[in]  aString            value에 저장될 입력 문자열
 * @param[in]  aStringLength      aString의 길이 
 * @param[in]  aPrecision         precision
 * @param[in]  aScale             scale
                                               ...
                                               stlErrorStack        * aErrorStack )
{
    ...
    *aSuccessWithInfo = STL_FALSE;
    ...
    sToDateTimeFormatInfo = aVectorFunc->mGetDateFormatInfoFunc( aVectorArg );    
...
/**
 * @brief session format 정보를 이용하여 Utf32 String으로부터 Date 타입의 value 구성
 * @param[in]  aString            value에 저장될 입력 문자열
 * @param[in]  aStringLength      aString의 길이 
 * @param[in]  aPrecision         precision
 * @param[in]  aScale             scale
...
/**
 * @brief Date value를 utf16 문자열로 변환
 * @param[in]  aValue          dtlDataValue
 * @param[in]  aPrecision      precision
 * @param[in]  aScale          scale
 * @param[in]  aAvailableSize  aBuffer에 사용할 수 있는 메모리 공간의 크기
...
/**
 * @brief Date value를 utf32 문자열로 변환
 * @param[in]  aValue          dtlDataValue
 * @param[in]  aPrecision      precision
 * @param[in]  aScale          scale
 * @param[in]  aAvailableSize  aBuffer에 사용할 수 있는 메모리 공간의 크기
