...
#ifdef STC_HAVE_GETPASS
#ifdef STC_HAVE_PWD_H
#include <pwd.h>
#endif
#else
#include <sys/ioctl.h>
#ifdef STC_HAVE_TERMIOS_H
#include <termios.h>
#define TERMIO struct termios
#else
#ifdef STC_HAVE_TERMIO_H
#include <termio.h>
...
#ifdef STC_HAVE_GETPASSPHRASE /* For Solaris */
...
#ifndef STC_HAVE_GETPASS

static void strGetPass( stlChar       * aBuffer,
                          ...
                          stlErrorStack * aErrorStack )
{
...
#ifdef STC_HAVE_GETPASS
    sPassword = getpass( aPrompt ? aPrompt : "Enter password: ");

    sPasswordSize = stlStrlen( sPassword );

    /* copy the password to buff and clear original (static) buffer */

    stlStrncpy( aBuffer, sPassword, aBufferLength );

    STL_TRY_THROW( aBufferLength > sPasswordSize, RAMP_ERR_STRING_TRUNCATION );

    stlMemset( sPassword, 0x00, sPasswordSize );
#else

    STL_TRY( stlOpenStdinFile( &sStdin, aErrorStack ) );
    STL_TRY( stlOpenStdoutFile( &sStdout, aErrorStack ) );
    
    fputs( aPrompt ? aPrompt : "Enter password: ", stdout );
    fflush(stdout);
    
#if defined(STC_HAVE_TERMIOS_H)
    tcgetattr( fileno(stdin), &sOrg );
    
    sTmp = sOrg;
    sTmp.c_lflag    &= ~(ECHO | ISIG | ICANON);
    sTmp.c_cc[VMIN]  = 1;
    sTmp.c_cc[VTIME] = 0;
    
    tcsetattr(fileno(stdin), TCSADRAIN, &sTmp);
    
    strGetPass( aBuffer, aBufferLength - 1, &sStdin, aEcho );
    
    tcsetattr( fileno(stdin), TCSADRAIN, &sOrg );
#elif defined(STC_HAVE_TERMIO_H)
    ioctl( fileno(stdin), (int)TCGETA, &sOrg );
    ...
    ioctl( fileno(stdin),(int)TCSETA, &sTmp );
    
    strGetPass( aBuffer, aBufferLength - 1, &sStdin, aEcho );
    
    ioctl(fileno(stdin),(int) TCSETA, &sOrg);
#else
    gtty( fileno(stdin), &sOrg );
    
    sTmp = sOrg;
    sTmp.sg_flags &= ~ECHO;
    sTmp.sg_flags |= RAW;
    
    stty( fileno(stdin), &sTmp );
    
    strGetPass( aBuffer, aBufferLength - 1, &sStdin, aEcho );
    
    stty( fileno(stdin), &sOrg );
#endif
    if( aEcho == STL_TRUE )
    {
        fputc( '\n', stdout );
    ...
    STL_CATCH( RAMP_ERR_STRING_TRUNCATION )
    {
        stlPushError( STL_ERROR_LEVEL_ABORT,
