...
/**
 * @file qlnoHashInstantAccess.c
 * @brief SQL Processor Layer Code Optimization Node - HASH INSTANT ACCESS
 */
...
#if 0
/**
 * @brief Hash Instant에 대한 Code Optimize를 수행한다.
                                       ...
                                       qllEnv               * aEnv )
{
    ...
    STL_PARAM_VALIDATE( aCodeOptParamInfo->mCandPlan->mPlanType == QLNC_PLAN_TYPE_HASH_INSTANT,
    ...
    sOptHashInstant->mTotalFilterExpr = sPlanHashInstant->mFilterExpr;
...
#if 0
/**
 * @brief HASH INSTANT ACCESS의 Optimization Node 정보를 완성한다.
                                        ...
                                        qllEnv                * aEnv )
{
    ...
    qlvInitInstantNode * sIniHashInstant     = NULL;
    ...
    for( sLoop = 0 ; sLoop < sOptHashInstant->mInstantNode->mRecColCnt ; sLoop++, sIdx++ )
    {
        ...
        {
            sColExprStackList[ sIdx ] = & sColExprStack[ sIdx ];
            sOptHashInstant->mNeedEvalRecCol = STL_TRUE;
        }
    ...
    sIniHashInstant = sOptHashInstant->mInstantNode;
...
#if 0
/**
 * @brief HASH INSTANT ACCESS의 All Expr 리스트 정보를 완성한다.
 * @param[in]     aDBCStmt        DBC Statement
 * @param[in]     aSQLStmt        SQL Statement 객체
 * @param[in]     aOptNodeList    Optimization Node List
 * @param[in,out] aOptNode        Optimization Node
