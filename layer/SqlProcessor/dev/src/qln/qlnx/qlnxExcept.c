...
/**
 * @file qlnxExcept.c
 * @brief SQL Processor Layer Execution Node - EXCEPT ( ALL / DISTINCT )
 */
...
/**
 * @brief EXCEPT (ALL / DISTINCT) node 를 초기화한다.
                                ...
                                qllEnv                * aEnv )
{
    ...
    STL_PARAM_VALIDATE( (aOptNode->mType == QLL_PLAN_NODE_EXCEPT_ALL_TYPE) ||
    ...
    if ( aOptNode->mType == QLL_PLAN_NODE_EXCEPT_DISTINCT_TYPE )
    {
...
/**
 * @brief EXCEPT (ALL / DISTINCT) node 를 수행한다.
 * @param[in]  aTransID      Transaction ID
 * @param[in]  aStmt         Statement
 * @param[in]  aDBCStmt      DBC Statement
 * @param[in]  aSQLStmt      SQL Statement
...
/**
 * @brief EXCEPT (ALL / DISTINCT) node 를 Fetch 한다.
                           ...
                           qllEnv                * aEnv )
{
    ...
    STL_PARAM_VALIDATE( (aFetchNodeInfo->mOptNode->mType == QLL_PLAN_NODE_EXCEPT_ALL_TYPE) ||
                        (aFetchNodeInfo->mOptNode->mType == QLL_PLAN_NODE_EXCEPT_DISTINCT_TYPE),
                        QLL_ERROR_STACK(aEnv) );
    ...
    if ( sInstantTable->mIsNeedBuild == STL_TRUE )
    {
        ...
        STL_TRY_THROW ( sInstantTable->mRecCnt > sSkipCnt, RAMP_EXIT );
        
        /*******************************************************
         * Other Child 를 반복 수행하며 Instant Table 에서 제거 
         *******************************************************/
        ...
        for( i = 1; i < sOptSetOP->mChildCount; i++ )
        {
            ...
            /**
             * Child Data 를 Fetch 하여 Instant Table 에서 삭제
             */
            ...
            while ( sChildEOF == STL_FALSE )
            {
                ...
                if ( sUsedBlockCnt > 0 )
                {
                    ...
                    for ( sBlockIdx = 0; sBlockIdx < sUsedBlockCnt; sBlockIdx++ )
                    {
                        ...
                        STL_DASSERT( sVersionConflict == STL_FALSE );
                    }
    ...
    else
    {
        ...
        *aEOF = sExeSetOP->mFetchInfo.mIsEndOfScan;
...
/**
 * @brief EXCEPT (ALL / DISTINCT) node 수행을 종료한다.
 * @param[in] aOptNode      Optimization Node
 * @param[in] aDataArea     Data Area 
 * @param[in] aEnv          Environment
 * @remarks
