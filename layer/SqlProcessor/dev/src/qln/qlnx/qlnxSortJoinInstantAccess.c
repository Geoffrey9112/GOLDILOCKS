...
/**
 * @file qlnxSortJoinInstantAccess.c
 * @brief SQL Processor Layer Execution Node - SORT JOIN INSTANT TABLE
 */
...
/**
 * @brief SORT JOIN INSTANT TABLE node를 초기화한다.
...
/**
 * @brief SORT JOIN INSTANT TABLE node를 수행한다.
 * @param[in]      aTransID      Transaction ID
 * @param[in]      aStmt         Statement
 * @param[in]      aDBCStmt      DBC Statement
 * @param[in,out]  aSQLStmt      SQL Statement
                                            ...
                                            qllEnv                * aEnv )
{
    ...
    STL_DASSERT( sExeSortInstant->mPreRangeEvalInfo == NULL );
    STL_DASSERT( sExeSortInstant->mPreKeyFilterEvalInfo == NULL );
    STL_DASSERT( sExeSortInstant->mPreFilterEvalInfo == NULL );
    ...
    STL_DASSERT( sFetchInfo->mRange->mMinRange == NULL );
    ...
    STL_DASSERT( sFetchInfo->mRange->mMaxRange == NULL );
    ...
    STL_DASSERT( sFetchInfo->mPhysicalFilter == NULL );
    ...
    /* Filter 가 없기 때문에 false filter 가 존재할 수 없다. */
    sExeSortInstant->mHasFalseFilter = STL_FALSE;
    ...
    /*     { */
    /*         sExeSortInstant->mHasFalseFilter = */
    /*             qlfHasFalseFilter( sOptSortInstant->mTableLogicalFilterExpr, */
    /*                                aDataArea ); */
    /*     } */
    /* } */
    /* else */
                                              ...
                                              qllEnv                * aEnv )
{
    ...
    /* filter 가 없기 때문에 false filter 처리가 불가 */
    ...
    if( sInstantTable->mIsNeedBuild == STL_TRUE )
    {
        ...
        if( sInstantTable->mRecCnt > 0 )
        {
            ...
            {
                KNL_SET_ALL_BLOCK_USED_CNT( sExeSortInstant->mReadColBlock, 0 );
                *aEOF = STL_TRUE;
                STL_THROW( RAMP_EXIT );
            }
            else
    ...
    QLL_OPT_ADD_TIME( sExeSortInstant->mCommonInfo.mBuildTime, sBuildTime - sExceptTime );
...
/**
 * @brief SORT JOIN INSTANT TABLE node를 Next Group으로 이동한다.
 * @param[in]      aTransID      Transaction ID
 * @param[in]      aStmt         Statement
 * @param[in]      aDBCStmt      DBC Statement
 * @param[in,out]  aSQLStmt      SQL Statement
                                              ...
                                              qllEnv                * aEnv )
{
    ...
    STL_DASSERT( sInstantTable->mIsNeedBuild == STL_FALSE );
    ...
    *aEOF = sFetchInfo->mIsEndOfScan;
    ...
    QLL_OPT_ADD_ELAPSED_TIME( sExeSortInstant->mCommonInfo.mFetchTime, sBeginFetchTime, sEndFetchTime );
    QLL_OPT_ADD_COUNT( sExeSortInstant->mCommonInfo.mFetchCallCount, 1 );
...
/**
 * @brief SORT JOIN INSTANT TABLE node를 현재 Group의 처음 위치로 Reset한다.
 * @param[in]      aTransID      Transaction ID
 * @param[in]      aStmt         Statement
 * @param[in]      aDBCStmt      DBC Statement
 * @param[in,out]  aSQLStmt      SQL Statement
...
//    sOptSortInstant = (qlnoInstant *) aOptNode->mOptNode;
    ...
    /* Reset Group */
    sFetchInfo->mIsEndOfScan = STL_FALSE;
    STL_TRY( smlResetGroup( sInstantTable->mIterator,
                            SML_ENV(aEnv) )
             == STL_SUCCESS );
...
/**
 * @brief SORT JOIN INSTANT TABLE node에 Flag를 Setting한다.
 * @param[in]      aStmt         Statement
 * @param[in]      aOptNode      Optimization Node
 * @param[in]      aDataArea     Data Area (Data Optimization 결과물)
 * @param[in]      aSetFlagBlock Flag를 Setting 정보를 갖는 Block
 * @param[in]      aEnv          Environment
 */
...
/**
 * @brief SORT JOIN INSTANT TABLE node의 Fetch Info에 Row Flag를 Setting한다.
 * @param[in]      aStmt         Statement
 * @param[in]      aOptNode      Optimization Node
 * @param[in]      aDataArea     Data Area (Data Optimization 결과물)
 * @param[in]      aRowFlag      Setting할 Row Flag
 * @param[in]      aEnv          Environment
 */
...
/**
 * @brief SORT JOIN INSTANT TABLE node를 Fetch한다.
                                          ...
                                          qllEnv                * aEnv )
{
    ...
    STL_PARAM_VALIDATE( aFetchNodeInfo->mOptNode->mType == QLL_PLAN_NODE_SORT_JOIN_INSTANT_ACCESS_TYPE,
                        QLL_ERROR_STACK(aEnv) );
...
/**
 * @brief SORT JOIN INSTANT TABLE node 수행을 종료한다.
 * @param[in]      aOptNode      Optimization Node
 * @param[in]      aDataArea     Data Area (Data Optimization 결과물)
 * @param[in]      aEnv          Environment
 *
