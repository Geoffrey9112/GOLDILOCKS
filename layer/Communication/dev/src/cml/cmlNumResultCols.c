...
/**
 * @brief NumParams Command Protocol을 기록한다.
...
{
    stlInt16   sCommandType = CML_COMMAND_NUMRESULTCOLS;
    cmpCursor  sCursor;
...
/**
 * @brief NumResultCols Result Protocol을 읽는다.
 * @param[in]  aHandle        Communication Handle
 * @param[out] aIgnored       Ignore protocol 여부
 * @param[out] aColumnCnt     Column Count
 * @param[in]  aErrorStack    Error Stack Pointer
 * 
 * @par protocol sequence:
 * @code
                                      ...
                                      stlErrorStack  * aErrorStack )
{
    ...
    CMP_READ_VAR_INT32( aHandle, &sCursor, &sColumnCnt, aErrorStack );
