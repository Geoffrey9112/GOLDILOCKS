...
{
    stlInt16   sCommandType = CML_COMMAND_NUMPARAMS;
    cmpCursor  sCursor;
...
/**
 * @brief NumParams Result Protocol을 읽는다.
 * @param[in]  aHandle        Communication Handle
 * @param[out] aIgnored       Ignore protocol 여부
 * @param[out] aParamCnt      Paramter Count
 * @param[in]  aErrorStack    Error Stack Pointer
 * 
 * @par protocol sequence:
 * @code
                                  ...
                                  stlErrorStack  * aErrorStack )
{
    ...
    CMP_READ_VAR_INT32( aHandle, &sCursor, &sParamCnt, aErrorStack );
