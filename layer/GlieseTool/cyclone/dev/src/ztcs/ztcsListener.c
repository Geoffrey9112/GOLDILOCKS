...
stlStatus ztcsCreateListener( stlErrorStack * aErrorStack )
{
    ...
    while( 1 )
    {
        ...
        if( stlPollContext( &(sCMMgr->mListenContext),
                            ...
                            aErrorStack ) == STL_SUCCESS )
        {
            STL_TRY( ztcsInitProtocol( &sNewContext,
                                       &sClientType,
                                       aErrorStack ) == STL_SUCCESS );
            ...
            switch( sClientType )
            {
                case 's':       //Slave
                {
                    if( sCMMgr->mSenderStatus != STL_TRUE )
                    {
                        ...
                        STL_TRY( ztcmSendProtocolHdrNData( &gMasterMgr->mCMMgr.mSenderContext,
                                                           sSndHeader,
                                                           NULL,
                                                           aErrorStack ) == STL_SUCCESS );
                        ...
                        STL_TRY( ztctCreateSenderThread( aErrorStack ) == STL_SUCCESS );
                    }
                    else
                    {
                        STL_TRY( ztcmLogMsg( aErrorStack,
                                             "[LISTENER] Slave connected but already slave exist.\n" ) == STL_SUCCESS );
                        /**
                         * 에러 메시지 보내고 Finalize Context ...
                         */
                        sSndHeader.mCategory = ZTC_CATEGORY_ERROR;
                        sSndHeader.mType     = ZTC_ERROR_SLAVE_ALREADY_CONNECTED;
                        sSndHeader.mLength   = 0;
            
                        STL_TRY( ztcmSendProtocolHdrNData( &sNewContext,
                                                           sSndHeader,
                                                           NULL,
                                                           aErrorStack ) == STL_SUCCESS );
                ...
                case 'm':       //Monitor
                {
                    if( sCMMgr->mMonitorStatus != STL_TRUE )
                    {
                        ...
                        STL_TRY( ztcmSendProtocolHdrNData( &gMasterMgr->mCMMgr.mMonitorContext,
                                                           sSndHeader,
                                                           NULL,
                                                           aErrorStack ) == STL_SUCCESS );
