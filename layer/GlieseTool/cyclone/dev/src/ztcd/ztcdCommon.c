                                  ...
                                  stlErrorStack       * aErrorStack )
{
    ...
    while( 1 )
    {
        ...
        stlStrncpy( sColumnMeta->mColumnName, sColumnName, stlStrlen( sColumnName ) + 1 );
        stlStrncpy( sColumnMeta->mDataType, sDataTypeStr, stlStrlen( sDataTypeStr ) + 1 );
        sColumnMeta->mIsPrimary  = sIsPrimaryKey;
        sColumnMeta->mIsUnique   = sIsUnique;
        sColumnMeta->mIsNullable = sIsNullable;
                              ...
                              stlErrorStack * aErrorStack )
{
    ...
    stlChar     sSqlState[ZTC_COMMAND_BUFFER_SIZE];
    SQLHSTMT    sStmtHandle;
    stlInt32    sState = 0;
    SQLRETURN   sResult;
    ...
    stlSnprintf( sSqlMeta,
                 ZTC_COMMAND_BUFFER_SIZE,
                 "DELETE FROM CYCLONE_SLAVE_META WHERE GROUP_NAME='%s'",
                 gConfigure.mCurrentGroupPtr->mName );
    
    stlSnprintf( sSqlState,
                 ZTC_COMMAND_BUFFER_SIZE,
...
stlStatus ztcdCreateMetaTable4Slave( stlErrorStack * aErrorStack )
{
                   ...
                   stlStrlen( sSqlString ) ) != SQL_SUCCESS )
    {
        stlSnprintf( sCreateString,
                     ZTC_COMMAND_BUFFER_SIZE,
                     "CREATE TABLE CYCLONE_SLAVE_META "
                   ...
                   stlStrlen( sSqlString ) ) != SQL_SUCCESS )
    {
        stlSnprintf( sCreateString,
                     ZTC_COMMAND_BUFFER_SIZE,
                     "CREATE TABLE CYCLONE_SLAVE_STATE "
                     "( GROUP_NAME VARCHAR(128), APPLIER_ID INTEGER, RESTART_LSN BIGINT, LASTCOMMIT_LSN BIGINT, "
                     "FILESEQ_NO BIGINT, GROUPID INTEGER, BLOCKSEQ INTEGER, LOGNO INTEGER, WRAPNO SMALLINT )" );
