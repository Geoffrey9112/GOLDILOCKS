...
stlStatus ztcmInitializeEnv4Master( stlErrorStack * aErrorStack )
{
     ...
     */
    STL_RING_INIT_HEADLINK( &gMasterMgr->mWaitWriteCkList,
                            STL_OFFSETOF( ztcChunkItem, mLink ) );
    
    STL_RING_INIT_HEADLINK( &gMasterMgr->mWriteCkList,
                            STL_OFFSETOF( ztcChunkItem, mLink ) );
            
    gMasterMgr->mWaitWriteState = STL_FALSE;

    ztcmInitSpinLock( &(gMasterMgr->mWriteSpinLock) );
    
    STL_TRY( stlCreateSemaphore( &(gMasterMgr->mWaitWriteSem),
                                 "cwwSem",
                                 0,
                                 aErrorStack ) == STL_SUCCESS );
    sState = 6;
     ...
     */
    STL_RING_INIT_HEADLINK( &gMasterMgr->mWaitReadCkList,
                            STL_OFFSETOF( ztcChunkItem, mLink ) );
    
    STL_RING_INIT_HEADLINK( &gMasterMgr->mReadCkList,
                            STL_OFFSETOF( ztcChunkItem, mLink ) );
    
    gMasterMgr->mWaitReadState = STL_FALSE;

    ztcmInitSpinLock( &(gMasterMgr->mReadSpinLock) );
    
    STL_TRY( stlCreateSemaphore( &(gMasterMgr->mWaitReadSem),
                                 "swrSem",
                                 0,
                                 aErrorStack ) == STL_SUCCESS );
    sState = 7;
    ...
    switch( sState )
    {
        case 8:
            (void)stlDestroySemaphore( &(gMasterMgr->mCaptureStartSem),
                                       aErrorStack );
        case 7:
            (void)stlDestroySemaphore( &(gMasterMgr->mWaitReadSem),
                                       aErrorStack );
            
            ztcmFinSpinLock( &(gMasterMgr->mReadSpinLock) );
            
        case 6:
            (void)stlDestroySemaphore( &(gMasterMgr->mWaitWriteSem),
                                       aErrorStack );
            
            ztcmFinSpinLock( &(gMasterMgr->mWriteSpinLock) );
        case 5:
...
stlStatus ztcmFinalizeEnv4Master( stlErrorStack * aErrorStack )
{
    ...
    STL_TRY( stlDestroySemaphore( &(gMasterMgr->mWaitReadSem),
                                  aErrorStack ) == STL_SUCCESS );
    
    ztcmFinSpinLock( &(gMasterMgr->mReadSpinLock) );
    
    STL_TRY( stlDestroySemaphore( &(gMasterMgr->mWaitWriteSem),
                                  aErrorStack ) == STL_SUCCESS );
...
stlStatus ztcmInitializeEnv4Slave( stlErrorStack * aErrorStack )
{
    ...
    gSlaveMgr->mApplierCount = ztcmGetApplierCount();
    sChunkCount              = ztcmGetCommChunkCount();
    ...
    STL_TRY( ztcdSelectCommitState( aErrorStack ) == STL_SUCCESS );
                            ...
                            stlErrorStack * aErrorStack )
{
    ...
    if( sTableMeta == NULL )
    {
        ...
        STL_RING_INIT_HEADLINK( &(sTableMeta->mColumnMetaList),
                                STL_OFFSETOF( ztcMasterColumnMeta, mLink ) );
                              ...
                              stlErrorStack  * aErrorStack )
{
    ...
    STL_RING_FOREACH_ENTRY( &gMasterMgr->mLogFileList, sLogFileInfo )
    {
        if( sLogFileInfo->mCaptureInfo.mRedoLogGroupId == aCaptureInfo->mRedoLogGroupId )
        {
            ...
            gMasterMgr->mCaptureMgr.mRecoveryLastCommitLSN = aCaptureInfo->mLastCommitLSN;
        }
        else
        {
                         ...
                         stlErrorStack  * aErrorStack )
{
    ...
    if( gConfigure.mIsMaster == STL_TRUE )
    {
        stlSnprintf( sFileName,
                     STL_MAX_FILE_PATH_LENGTH,
                     "%strc"STL_PATH_SEPARATOR"cyclone_master_%s.trc",
                     gConfigure.mHomeDir,
                     gConfigure.mCurrentGroupPtr->mName );
    }
    else
    {
        stlSnprintf( sFileName,
                     STL_MAX_FILE_PATH_LENGTH,
                     "%strc"STL_PATH_SEPARATOR"cyclone_slave_%s.trc",
                     gConfigure.mHomeDir,
                     gConfigure.mCurrentGroupPtr->mName );
    }
...
{
    STL_ASSERT( *aSpinLock == ZTC_SPINLOCK_STATUS_INIT );
}
...
void ztcmReleaseSpinLock( ztcSpinLock * aSpinLock )
{
    ...
    STL_ASSERT( sRetVal == ZTC_SPINLOCK_STATUS_LOCKED );
}


dtlCharacterSet ztcmGetCharSetIDFunc( void * aArgs )
{
    STL_ASSERT( gSlaveMgr != NULL );
...
stlStatus ztcmReadMasterTableMeta( stlErrorStack * aErrorStack )
{
    ...
    ztcMasterTableMeta  * sTableMeta  = NULL;
    ztcMasterColumnMeta * sColumnMeta = NULL;
    ...
    if( sExist == STL_TRUE )
    {
        ...
        while( 1 )
        {
            ...
            */
            ...
            {
                STL_TRY( stlAllocRegionMem( &(gMasterMgr->mRegionMem),
                                            STL_SIZEOF( ztcMasterColumnMeta ),
                                            (void**)&sColumnMeta,
                                            aErrorStack ) == STL_SUCCESS );
                
                STL_RING_INIT_DATALINK( sColumnMeta,
                                        STL_OFFSETOF( ztcMasterColumnMeta, mLink ) );
                
                STL_TRY( stlReadFile( &sFile,
                                      &(sColumnMeta->mColumnName),
                                      STL_SIZEOF(ztcMasterColumnMeta) - STL_SIZEOF(stlRingEntry),
                                      &sReadSize,
                                      aErrorStack ) == STL_SUCCESS );
                
                STL_DASSERT( sReadSize == STL_SIZEOF(ztcMasterColumnMeta) - STL_SIZEOF(stlRingEntry) );
                           ...
                           stlErrorStack * aErrorStack )
{
    ...
    if( aIsMaster == STL_TRUE )
    {
        stlPrintf("|       CYCLONE STATUS - MASTER      |\n");
    }
    else
    {
        stlPrintf("        CYCLONE STATUS - SLAVE        \n");
    }
    stlPrintf("======================================\n");
...
stlStatus ztcmGetSlaveIp( stlChar       * aSlaveIp,
                          stlErrorStack * aErrorStack )
{
    stlInt32 sLen = stlStrlen( gMonitorInfo.mSlaveIp );
    
    STL_TRY_THROW( sLen > 0 && sLen < 32, 
                   RAMP_ERR_INVALID_SLAVE_IP );
    
    stlStrncpy( aSlaveIp, gMonitorInfo.mSlaveIp, sLen + 1 );
...
stlStatus ztcmLogConfigureMsgSlave( stlErrorStack * aErrorStack )
{
    ...
    sApplierCount      = gConfigure.mApplierCount == 0 ?
        ZTC_APPLIER_SESSION_COUNT : gConfigure.mApplierCount;

    sApplierCommitSize = gConfigure.mApplierCommitSize == 0 ?
        ZTC_APPLIER_TRANS_COMMIT_MAX_SIZE : gConfigure.mApplierCommitSize;

    sUpdateApplyMode   = gConfigure.mUpdateApplyMode;
