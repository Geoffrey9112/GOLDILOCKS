                             ...
                             stlErrorStack       * aErrorStack )
{
    ...
    STL_RING_FOREACH_ENTRY( &aTableInfo->mColumnList, sColumnValue )
    {
        if( sColIdx >= aColIdx )
        {
            ZTC_READ_COLUMN_VALUE_LEN( &sPtr, &sColValueLen );
                ...
                sColumnValue->mInfoPtr->mDataTypeId == DTL_TYPE_LONGVARBINARY )
            {
                ...
                ZTC_READ_COLUMN_AND_MOVE_PTR( &sPtr,
                                              sVariableStr->arr,
                                              &sColValueLen );
                                          ...
                                          stlErrorStack       * aErrorStack )
{
    ...
    STL_TRY_THROW( ztcmGetUpdateApplyMode() >= 1, RAMP_SUCCESS );   //해당 옵션을 사용하는지 확인하는 과정..
    
    STL_READ_MOVE_INT16( &sColCount, sPtr, sOffset );
    
    STL_TRY_THROW( sColCount > 0, RAMP_SUCCESS );
    
    STL_READ_MOVE_INT32( &sColOrd, sPtr, sOffset );
    sPtr += sOffset;
                                    ...
                                    stlErrorStack       * aErrorStack )
{
    ...
    STL_READ_MOVE_INT64( &sTransId, aData, sOffset );
    STL_READ_MOVE_INT64( &sScn, aData, sOffset );
    STL_READ_MOVE_INT16( &sTmpDstPieceHdrSize, aData, sOffset );
    sOffset += sTmpDstPieceHdrSize;
    
    STL_READ_MOVE_INT32( &sFirstColOrd, aData, sOffset );
    STL_READ_MOVE_INT16( &sAppendPieceColCount, aData, sOffset );
    STL_READ_MOVE_INT16( &sAppendPieceBodySize, aData, sOffset );
    sPtr = aData + sOffset;
