                        ...
                        stlErrorStack * aErrorStack )
{
    ...
    /**
     * mMsgIdArray 메모리가 부족하면 다시 할당한다.
     */
    if( aApplierMgr->mMsgIdCnt >= aApplierMgr->mMsgIdBuffCnt )
    {
        sTempPtr = aApplierMgr->mMsgIdArray;
        ...
        stlMemcpy( aApplierMgr->mMsgIdArray, sTempPtr, aApplierMgr->mMsgIdCnt * STL_SIZEOF( stlUInt64 ) );
        stlFreeHeap( sTempPtr );
    }

    aApplierMgr->mMsgIdArray[aApplierMgr->mMsgIdCnt++] = aMsgId;
                           ...
                           stlErrorStack * aErrorStack )
{
    ...
    stlInt32              sCommentLen;
    stlInt32              sCheckState = 2;
    ...
    while( 1 )
    {
        ...
        if( sDoRead == STL_FALSE )
        {
            ...
            STL_TRY( ztcaReadDataFromChunk( aApplierMgr,
                                            ...
                                            aErrorStack ) == STL_SUCCESS );
//            stlDebug("ztcaReadDataFromChunk : sTransHdr.mSize %x \n", sTransHdr.mSize);

            STL_TRY( ztcaAddMsgId( aApplierMgr, sTransHdr.mMsgId, aErrorStack ) == STL_SUCCESS );
        }
        ...
        switch( sTransHdr.mLogType )
        {
            case ZTC_CAPTURE_TYPE_BEGIN:
            {
                if( sCheckState != 2 )
                {
                    ztcmLogMsg( aErrorStack, "Begin Comes without Commit[%d]\n", aApplierMgr->mAppId );
                }
                sCheckState = 1;
                STL_TRY( ztcaReadDataFromChunk( aApplierMgr,
                                                sTransData,
                ...
                {
                    ZTC_READ32_BYTEORDER( gSlaveMgr->mLogByteOrder,
                                          &sCommentLen,
                                          sTransData );

                    STL_TRY( ztcaTxBeginToFile( aApplierMgr,
                                                sTransData + STL_SIZEOF(sCommentLen),
                                                sCommentLen,
                                                aErrorStack )
                             == STL_SUCCESS );
                }
            ...
            case ZTC_CAPTURE_TYPE_COMMIT:
            {
                if( sCheckState != 1 )
                {
                    ztcmLogMsg( aErrorStack, "Commit Comes without Begin[%d]\n", aApplierMgr->mAppId );
                }
                sCheckState = 2;

                stlDebug("ZTC_CAPTURE_TYPE_COMMIT \n");
                ...
                STL_TRY( ztcmChangeByteOrderCaptureInfo( gSlaveMgr->mLogByteOrder,
                                                         sCaptureInfo,
                                                         aErrorStack )
                         == STL_SUCCESS );
            ...
            {
                stlDebug("ZTC_CAPTURE_TYPE_INSERT \n");
            ...
            {
                stlDebug("ZTC_CAPTURE_TYPE_SUPPL_DEL \n");
            ...
            {
                stlDebug("ZTC_CAPTURE_TYPE_SUPPL_UPD \n");

                STL_TRY( ztcaFindApplierTableInfo( aApplierMgr,
                                                   &sTableInfo,
                                                   sTransHdr.mTableId,
                                                   aErrorStack ) == STL_SUCCESS );
                stlDebug("slave table name %s \n", sTableInfo->mInfoPtr->mSlaveTable);
                if( sTableInfo != NULL )
                {
                    if( ztcaCheckNeedExecute( aApplierMgr,
                                              sTableInfo,
                    ...
                    while( 1 )
                    {
                        ...
                        STL_TRY( ztcaAddMsgId( aApplierMgr, sTmpTransHdr.mMsgId, aErrorStack ) == STL_SUCCESS );
                          ...
                          stlErrorStack  * aErrorStack )
{
    ...
    while( 1 )
    {
        ...
        STL_DASSERT( sItem->mRefCount >= sAppItem->mRefCount );
                      ...
                      stlInt32        aSlotIdx )
{
    ...
    STL_DASSERT( aSlotIdx < ZTC_DISTRIBUTOR_SLOT_SIZE );
