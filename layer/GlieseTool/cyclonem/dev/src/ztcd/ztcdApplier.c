                                ...
                                stlErrorStack        * aErrorStack )
{
    ...
    stlDebug("ztcdDoInsertNExecute start \n");
    ...
    {
        stlDebug("ztcdDoInsertNExecute %s, %ld \n",
    ...
    stlDebug("ztcdDoInsertNExecute 2 \n");

    if( gConfigure.mApplyToFile == STL_TRUE )
    {
        for( sIdx = 0; sIdx < sStmtInfo->mInsertIdx; sIdx++ )
        {
            STL_TRY( ztcaInsertToFile( aApplierMgr,
                                       aTableInfo,
                                       aErrorStack )
                     == STL_SUCCESS );
        }
                                 ...
                                 stlErrorStack  * aErrorStack )
{
    ...
    */
    ...
    if( gConfigure.mApplyToFile == STL_TRUE )
    {
        STL_TRY( ztcaCommitToFile( aApplierMgr,
                                   aCaptureInfo,
                                   aErrorStack )
                 == STL_SUCCESS );
    }
    else
    {
        ...
        stlSnprintf( sSqlString,
                     ...
                     "UPDATE CYCLONEM_SLAVE_STATE SET RESTART_LSN=%ld, LASTCOMMIT_LSN=%ld, FILESEQ_NO=%ld, GROUPID=%d, BLOCKSEQ=%d, LOGNO=%d, WRAPNO=%d "
    ...
    /**
     * commit된 packet에 대해서 ACK를 보낸다.
     */


    for( i = 0; i < aApplierMgr->mMsgIdCnt; i++ )
    {
        (void)gZtcLibFunc.mAckData( gSlaveMgr->mCMMgr.mSubContext, aApplierMgr->mMsgIdArray[i] );
    }

    stlAtomicInc64( &gCommitIdx );
    stlAtomicAdd64( &gMsgCnt, aApplierMgr->mMsgIdCnt );
                                ...
                                stlErrorStack        * aErrorStack )
{
    ...
    STL_RING_FOREACH_ENTRY( &aTableInfo->mColumnList, sColumnValue )
    {
        ...
        stlDebug("ztcdDoDeleteNExecute %s, %ld \n",
                 sColumnValue->mInfoPtr->mColumnName,
                 sColumnValue->mInfoPtr->mDataTypeId );
    ...
    if( gConfigure.mApplyToFile == STL_TRUE )
    {
        for( sIdx = 0; sIdx < sStmtInfo->mDeleteIdx; sIdx++ )
        {
            STL_TRY( ztcaDeleteToFile( aApplierMgr,
                                       aTableInfo,
                                       aErrorStack )
                     == STL_SUCCESS );
        }
                                ...
                                stlErrorStack        * aErrorStack )
{
    ...
    ztcStmtInfo    * sStmtInfo    = &aTableInfo->mStmtInfo;
    stlInt64         sArraySize   = aApplierMgr->mArraySize;
    stlInt32         sUpdateMode  = ztcmGetUpdateApplyMode( aErrorStack );
     ...
     */
    if( (aTableInfo->mNeedUpdatePrepare == STL_TRUE) && (gConfigure.mApplyToFile == STL_FALSE) )
    {
        /**
        ...
        STL_RING_FOREACH_ENTRY( &aTableInfo->mColumnList, sColumnValue )
        {
            stlDebug("Column mIsUpdated %d, %s \n", sColumnValue->mIsUpdated, sColumnValue->mInfoPtr->mColumnName);
...
//printf("ztcdSQLPrepare %s \n", sPrepareStr);
    ...
    STL_RING_FOREACH_ENTRY( &aTableInfo->mColumnList, sColumnValue )
    {
        ...
        stlDebug("ztcdDoUpdateNExecute1 %s, %ld \n",
                 sColumnValue->mInfoPtr->mColumnName,
                 sColumnValue->mInfoPtr->mDataTypeId );
    ...
    STL_RING_FOREACH_ENTRY( &aTableInfo->mColumnList, sColumnValue )
    {
        ...
        stlDebug("ztcdDoUpdateNExecute2 %s, %ld \n",
                 sColumnValue->mInfoPtr->mColumnName,
                 sColumnValue->mInfoPtr->mDataTypeId );
    ...
    if( gConfigure.mApplyToFile == STL_TRUE )
    {
        for( sIdx = 0; sIdx < sStmtInfo->mUpdateIdx; sIdx++ )
        {
            STL_TRY( ztcaUpdateToFile( aApplierMgr,
                                       aTableInfo,
                                       aErrorStack )
                     == STL_SUCCESS );
        }
                                    ...
                                    stlErrorStack     * aErrorStack )
{
    ...
    STL_TRY( ztcdSQLPrepare( sStmtHandle,
                             (SQLCHAR*)"INSERT INTO CYCLONEM_SLAVE_META VALUES( ?, ?, ?, ?, ?, ? )",
                             SQL_NTS,
                             aErrorStack ) == STL_SUCCESS );
