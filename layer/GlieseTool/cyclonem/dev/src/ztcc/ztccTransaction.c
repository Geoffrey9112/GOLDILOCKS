                                     ...
                                     stlErrorStack   * aErrorStack )
{
    ...
    if( STL_RING_IS_EMPTY( &aTransSlotHdr->mTransDataList ) == STL_TRUE )
    {
         ...
         */
        STL_DASSERT( aTransSlotHdr->mFileLogCount > 0 );
                                         ...
                                         sPrevTransData )
    {
        ...
        STL_DASSERT( sTransHdr->mRecordId == aRecordId );
...
stlStatus ztccAddTransDataToSlot( ztcTransSlotHdr * aTransSlotHdr,
                                  ...
                                  stlErrorStack   * aErrorStack )
{
    STL_DASSERT( aTransHdr->mSize <= ZTC_LOG_PIECE_MAX_SIZE );
                                        ...
                                        stlErrorStack * aErrorStack )
{
    ...
    while( sTableInfo != NULL )
    {
        if( sTableInfo->mGiveUpTransId == aTransId )
        {
            ...
            stlDebug( "[CAPTURE] DDL or Primary Key Update occured At %s.%s Table.\n",
