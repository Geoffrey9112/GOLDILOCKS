...
#ifdef __linux
/**
 * TODO lym standard layer로 옮겨야 하는데 다른 OS는 기능이 없는것 같음.
 */
#include <sys/inotify.h>
#endif


#define ZTCC_EVENT_SIZE         ( sizeof (struct inotify_event) )
#define ZTCC_EVENT_BUF_LEN      ( 1024 * ( ZTCC_EVENT_SIZE + 16 ) )
                       ...
                       stlErrorStack  * aErrorStack )
{
    ...
    stlFile           sNotiFd;
    stlPollFd         sPollFd;
    stlInt32          sWatchDesc     = -1;
    stlChar           sNotiBuff[ZTCC_EVENT_BUF_LEN];
    stlInt32          sNotiLen;
    stlInt32          sSigFdNum;
#else
...
#ifdef __linux
    sNotiFd.mFileDesc = inotify_init();
    STL_TRY_THROW( sNotiFd.mFileDesc >= 0, RAMP_ERR_NOTIFY );
    state = 1;

    sPollFd.mSocketHandle = sNotiFd.mFileDesc;
    sPollFd.mReqEvents = STL_POLLIN | STL_POLLERR;
    sPollFd.mRetEvents = 0;
#endif
    ...
    while( gRunState == STL_TRUE )
    {
...
#ifdef __linux
        if( stlStrcmp( sLogFileName, sLogFileInfo->mName ) != 0 )
        {
            if( sWatchDesc != -1 )
            {
                STL_TRY_THROW( inotify_rm_watch( sNotiFd.mFileDesc, sWatchDesc ) == 0,
                               RAMP_ERR_NOTIFY );
            }
            sWatchDesc = inotify_add_watch( sNotiFd.mFileDesc,
                                            sLogFileInfo->mName,
                                            IN_MODIFY | IN_CREATE | IN_DELETE | IN_MOVED_FROM | IN_MOVED_TO | IN_MOVE_SELF);
            STL_TRY_THROW( sWatchDesc >= 0, RAMP_ERR_NOTIFY );
        }
        ...
        if( sFirst == STL_FALSE )
        {
                         ...
                         aErrorStack ) == STL_FAILURE )
            {
                ...
                /**
                 * timeout이면 continue 하지 않고 밑으로 내려간다.
                 * log file오류인 경우 error처리되어야 함.
                 */
            }
            else
            {
                sNotiLen = read( sNotiFd.mFileDesc, sNotiBuff, ZTCC_EVENT_BUF_LEN);
                if( sNotiLen > 0 )
                {
                    /* do nothing */
                }
...
#ifdef __linux
    state = 0;
    (void)stlCloseFile( &sNotiFd, aErrorStack );
#endif

    return STL_SUCCESS;
    
#ifdef __linux
    STL_CATCH( RAMP_ERR_NOTIFY )
    {
        ...
        while( sLeftLogCount > 0 )
        {
            ...
            for( sIdx = 0; sIdx < sLogHdr.mLogPieceCount; sIdx++ )
            {
                ZTC_READ_MOVE_LOGPIECE_HDR( &sLogPieceHdr,
                                            sLogBody,
                                            sLogBodyOffset );
                STL_DASSERT( sLogPieceHdr.mSize <= ZTC_LOG_PIECE_MAX_SIZE );
                ...
                if( sLogPieceHdr.mComponentClass == ZTC_COMPONENT_TRANSACTION )
                {
                    switch( sLogPieceHdr.mType )
                    {
                        ...
                        case ZTC_LOG_COMMIT:
                        {
                            ...
                            if( aRecovery == STL_FALSE )
                            {
                                STL_TRY( ztccOperateTransCommit( sLogPieceBody,
                                                                 sTransId,
                                                                 sCommitSCN,
                                                                 &sProgress,
                                                                 aErrorStack ) == STL_SUCCESS );
                            ...
                            {
                                STL_DASSERT( 0 );
                                STL_TRY( ztccOperateTransRollback( sLogPieceBody,
                                                                   sTransId,
                                                                   sCommitSCN,
                                                                   &sProgress,
                                                                   aErrorStack ) == STL_SUCCESS );
                            ...
                            STL_TRY( ztccSaveRestartInfo( &sProgress, aErrorStack )
                                     == STL_SUCCESS );
            ...
            if( aRecovery == STL_FALSE )
            {
                STL_TRY( ztccDummyFunc( sLogHdr.mLsn,
                                        &sStatus ) == STL_SUCCESS );
            }
            else
            {
...
/**
 * restart시 다시 capture 시작할 LSN을 저장한다.
 *
 * 1. 현재 TR의 시작 LSN중 최소 값을 구해서 저장한다.
 * 2. 현재 TR이 없으면 aCurProgress(commit시 정보)를 저장한다.
 */
stlStatus ztccSaveRestartInfo( ztcCaptureProgress   * aCurProgress,
                               stlErrorStack        * aErrorStack )
{
    ztcRestartInfo     * sRestartInfo;
    ztcCaptureInfo       sCaptureInfo;
    ztcCaptureMgr      * sCaptureMgr = &(gMasterMgr->mCaptureMgr);
    ztcCaptureProgress * sProgress = NULL;

    /**
     * TR이 없으면 commit의 정보를 저장한다.
     * 제일 먼저 capture한 TR LSN이 최소 LSN이고
     * 이 값을 restart LSN으로 사용한다.
     */
    if( STL_RING_IS_EMPTY( &(sCaptureMgr->mRestartInfoList) ) == STL_TRUE )
    {
        sProgress = aCurProgress;
    }
    else
    {
    ...
    /**
     * mRecoveryLastCommitLSN이 저장되어 있는 LSN보다 크면
     */
    if( sProgress->mLSN > gMasterMgr->mCaptureMgr.mRecoveryLastCommitLSN )
    {
        sCaptureInfo.mRestartLSN     = sProgress->mLSN;
        sCaptureInfo.mBlockSeq       = sProgress->mBlockSeq;
        sCaptureInfo.mLastCommitLSN  = ZTC_INVALID_LSN;
        ...
        gMasterMgr->mCaptureMgr.mRecoveryLastCommitLSN = sCaptureInfo.mRestartLSN;

        if( gMasterMgr->mCaptureMgr.mSaveCount % 10 == 0 )    // TODO lym 성능확인
        {
            (void)gZtcLibFunc.mSaveData( gMasterMgr->mCMMgr.mPubContext, &sCaptureInfo, STL_SIZEOF(sCaptureInfo) );
        }
        gMasterMgr->mCaptureMgr.mSaveCount++;
    }
