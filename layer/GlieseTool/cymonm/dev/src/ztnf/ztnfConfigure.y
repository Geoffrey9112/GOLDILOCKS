...
%token ZTN_TOK_UPDATE_APPLY_MODE
%token ZTN_TOK_LIB_CONFIG;
%token ZTN_TOK_SQL_FILE_NAME;
%token ZTN_TOK_SQL_FILE_SIZE;
%token ZTN_TOK_MONITOR_FILE;
%token ZTN_TOK_MONITOR_CYCLE_MS;
...
apply_table_clause      : ZTN_TOK_APPLY_TABLE ZTN_TOK_EQUALS ZTN_TOK_OPEN_BRACKET applies_clause ZTN_TOK_CLOSE_BRACKET
                        | ZTN_TOK_APPLY_TABLE ZTN_TOK_EQUALS apply_clause
                        ;

applies_clause          : applies_clause ZTN_TOK_COMMA apply_clause
                        | apply_clause
                        ;
                                      
apply_clause            : ZTN_TOK_STRING ZTN_TOK_TO ZTN_TOK_STRING
                          {

                          }
                        | ZTN_TOK_STRING ZTN_TOK_DOT ZTN_TOK_STRING ZTN_TOK_TO ZTN_TOK_STRING
                          {

                          }
                        | ZTN_TOK_STRING ZTN_TOK_TO ZTN_TOK_STRING ZTN_TOK_DOT ZTN_TOK_STRING 
                          {

                          }
                        | ZTN_TOK_STRING ZTN_TOK_DOT ZTN_TOK_STRING ZTN_TOK_TO ZTN_TOK_STRING ZTN_TOK_DOT ZTN_TOK_STRING 
                          {
...
master_ip_clause        : ZTN_TOK_MASTER_IP ZTN_TOK_EQUALS ZTN_TOK_STRING
                          {

                          };
                          
trans_filePath_clause   : ZTN_TOK_TRANS_FILE_PATH ZTN_TOK_EQUALS ZTN_TOK_STRING
                          {

                          };

applier_count_clause    : ZTN_TOK_APPLIER_COUNT ZTN_TOK_EQUALS ZTN_TOK_NUMBER
                          {

                          };

apply_commit_clause     : ZTN_TOK_APPLY_COMMIT_SIZE ZTN_TOK_EQUALS ZTN_TOK_NUMBER
                          {

                          };

apply_array_clause      : ZTN_TOK_APPLY_ARRAY_SIZE ZTN_TOK_EQUALS ZTN_TOK_NUMBER
                          {

                          };

propagate_mode_clause   : ZTN_TOK_PROPAGATE_MODE ZTN_TOK_EQUALS ZTN_TOK_NUMBER
                          {

                          };

update_apply_mode_clause : ZTN_TOK_UPDATE_APPLY_MODE ZTN_TOK_EQUALS ZTN_TOK_NUMBER
                          {
...
lib_config_clause        : ZTN_TOK_LIB_CONFIG ZTN_TOK_EQUALS ZTN_TOK_STRING
                           {
                              if( gZtnBraceStarted == STL_FALSE )
                              {
                                  ztnfSetLibConfigToGlobalArea( aParam, $3 );
...
sql_file_name_clause    : ZTN_TOK_SQL_FILE_NAME ZTN_TOK_EQUALS ZTN_TOK_STRING
                           {

                           };
                           
sql_file_size_clause    : ZTN_TOK_SQL_FILE_SIZE ZTN_TOK_EQUALS ZTN_TOK_NUMBER
                           {
       
                           };

monitor_file_clause      : ZTN_TOK_MONITOR_FILE ZTN_TOK_EQUALS ZTN_TOK_STRING
                           {
                              if( gZtnBraceStarted == STL_FALSE )
                              {
                                  ztnfSetMonitorFileToGlobalArea( aParam, $3 );
...
monitor_cycle_ms_clause  : ZTN_TOK_MONITOR_CYCLE_MS ZTN_TOK_EQUALS ZTN_TOK_NUMBER
                           {
                              if( gZtnBraceStarted == STL_FALSE )
                              {
                                  ztnfSetMonitorCycleMsToGlobalArea( aParam, $3 );
                                   ...
                                   stlChar           * aLibConfig )
{
    ...
    if( ztnmSetLibConfigToGlobalArea( aLibConfig,
                                      sErrorStack ) != STL_SUCCESS )
    {
        aParam->mErrStatus  = STL_FAILURE;
    }
                                     ...
                                     stlChar           * aMonitorFile )
{
    ...
    if( ztnmSetMonitorFileToGlobalArea( aMonitorFile,
                                        sErrorStack ) != STL_SUCCESS )
    {
        aParam->mErrStatus  = STL_FAILURE;
    }
                                        ...
                                        stlChar           * aMonitorCycleMs )
{
    ...
    if( ztnmSetMonitorCycleMsToGlobalArea( aMonitorCycleMs,
                                           sErrorStack ) != STL_SUCCESS )
    {
        aParam->mErrStatus  = STL_FAILURE;
    }
