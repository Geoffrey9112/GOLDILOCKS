...
stlStatus ztnmInitializeEnv( stlErrorStack * aErrorStack )
{
    ...
    /**
     * SIGNAL을 등록하기전에 Alloc
     */
    STL_TRY( ztnmAllocEnvHandle( &gZtnManager.mEnvHandle,
                                 aErrorStack ) == STL_SUCCESS );
    sState = 2;
    ...
    switch( sState )
    {
        case 3:
            (void)stlSetSignalHandler( ZTN_SIGNAL_DEFINE,
                                       ...
                                       aErrorStack );
        case 2:
            (void) ztnmFreeEnvHandle( gZtnManager.mEnvHandle,
                                      aErrorStack );
        case 1:
...
stlStatus ztnmFinalizeEnv( stlErrorStack * aErrorStack )
{
    ...
    STL_TRY( ztnmFreeEnvHandle( gZtnManager.mEnvHandle,
                                aErrorStack ) == STL_SUCCESS );
                                     ...
                                     stlErrorStack * aErrorStack )
{
    ...
    stlStrncpy( gZtnConfigure.mUserId, aUserId, sLen + 1 );
                         ...
                         stlErrorStack * aErrorStack )
{
    ...
    if( sFound == STL_TRUE )
    {
        stlStrncpy( aUserId, sGroup->mUserId, sLen + 1 );
    }
    else
    {
        STL_TRY_THROW( stlStrlen( gZtnConfigure.mUserId ) > 0, RAMP_ERR_INVALID_VALUE );
        
        stlStrncpy( aUserId, gZtnConfigure.mUserId, stlStrlen( gZtnConfigure.mUserId ) + 1 );
    }
                                     ...
                                     stlErrorStack * aErrorStack )
{
    ...
    stlStrncpy( gZtnConfigure.mUserPw, aUserPw, sLen + 1 );
                         ...
                         stlErrorStack * aErrorStack )
{
    ...
    if( sFound == STL_TRUE )
    {
        stlStrncpy( aUserPw, sGroup->mUserPw, sLen + 1 );
    }
    else
    {
        STL_TRY_THROW( stlStrlen( gZtnConfigure.mUserPw ) > 0, RAMP_ERR_INVALID_VALUE );
        
        stlStrncpy( aUserPw, gZtnConfigure.mUserPw, stlStrlen( gZtnConfigure.mUserPw ) + 1 );
    }
...
stlStatus ztnmSetUserEncryptPwToGlobalArea( stlChar       * aUserEncryptPw,
                                            stlErrorStack * aErrorStack )
{
    stlChar     sPlainText[ZTN_MAX_CIPHER_SIZE];
    stlChar     sCipherText[ZTN_MAX_CIPHER_SIZE];
    stlSize     sCipherTextLen;
    stlSize     sPlainTextLen;

    STL_TRY_THROW( stlStrlen( gZtnConfigure.mUserPw ) == 0, RAMP_ERR_ALREADY_EXIST_USER_PW );

    STL_TRY_THROW( stlStrlen( gZtnConfigure.mKey ) != 0, RAMP_NO_KEY );

    sCipherTextLen = STL_SIZEOF( sCipherText );

    STL_TRY( ztnmBase64Decode( aUserEncryptPw, (stlUInt8 *)sCipherText, &sCipherTextLen, aErrorStack )
             == STL_SUCCESS );

    sPlainTextLen = STL_SIZEOF( sPlainText );
    STL_TRY( ztnmDecrypt( sCipherText, sCipherTextLen, gZtnConfigure.mKey, sPlainText, &sPlainTextLen, aErrorStack )
             == STL_SUCCESS );

    sPlainText[sPlainTextLen] = 0x00;

    STL_TRY( ztnmSetUserPwToGlobalArea( sPlainText, aErrorStack ) == STL_SUCCESS );
                                           ...
                                           stlErrorStack * aErrorStack )
{
    ...
    STL_TRY( ztnmSetUserPwToGroupArea( sPlainText, aErrorStack ) == STL_SUCCESS );
                         ...
                         stlErrorStack  * aErrorStack )
{
    ...
    stlSnprintf( sFileName,
                 STL_MAX_FILE_PATH_LENGTH,
                 "%strc"STL_PATH_SEPARATOR"cymon.trc",
                 gZtnConfigure.mHomeDir );
...
stlInt64 ztnmGetMonitorCycle()
{
    ...
    if( gZtnConfigure.mCycle != 0 )
    {
        sRet = gZtnConfigure.mCycle;
    }
    else
    {
        sRet = ZTN_DEFAULT_MONITOR_CYCLE;
    }
