...
IF /I "%1" == "bit" (
  IF "%2" == "32" SET BUILD_CONFIGURATION_TYPE="Visual Studio 12"&SET BUILD_BIT=32& GOTO :BIT_END
  IF "%2" == "64" SET BUILD_CONFIGURATION_TYPE="Visual Studio 12 Win64"& SET BUILD_BIT=64& GOTO :BIT_END
...
@ECHO ON
cmake -G %BUILD_CONFIGURATION_TYPE% -DCMAKE_BUILD_TYPE=%BUILD_TYPE% -DCMAKE_TARGET_HOST_TYPE=Client -DCMAKE_BITS=%BUILD_BIT% .
@ECHO OFF
